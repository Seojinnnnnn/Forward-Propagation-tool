...
(async()=>{
 ...
 const savedProject=evaluate('projectData()');context.dragProject=savedProject;assert.equal(evaluate('validateProject(dragProject).initialPositions.points.length'),pre);savedProject.initialPositions.points[0].x=Infinity;assert.throws(()=>evaluate('validateProject(dragProject)'));
 console.log('PASS: path creation, scaled pointer coordinates, selection/movement, manual layout replay, hidden-sphere selection, pointer cancellation, layout persistence validation.');
