...
(async()=>{
 ...
 document.head={append(link){queueMicrotask(()=>link.onload());}};document.fonts.load=async()=>[{}];elements.font.querySelector=()=>null;context.fontsLoaded=0;
 elements['google-family'].value='Noto Sans KR';await elements['google-connect'].onclick();assert.equal(evaluate('state.font'),'google');assert.equal(evaluate('googleFont.family'),'Noto Sans KR');assert.equal(evaluate('projectData().googleFont.family'),'Noto Sans KR');
 const previous=evaluate('googleFont.family');elements['google-family'].value='<bad>';await elements['google-connect'].onclick();assert.equal(evaluate('googleFont.family'),previous);assert(elements.notice.classList);
 document.fonts.load=async()=>[];elements['google-family'].value='Missing Family';await elements['google-connect'].onclick();assert.equal(evaluate('googleFont.family'),previous);assert.equal(evaluate('fontBusy'),false);assert.equal(elements['google-connect'].disabled,false);
 console.log('PASS: Google font load/application, saved family metadata, invalid-name rejection, missing-font fallback, and busy-state cleanup (mock font loader).');
