...
(async()=>{
 ...
 for(const value of evaluate('resolutions')){elements.resolution.value=value;context.formatValue=value;evaluate('setResolution(formatValue);resetWorld()');const [pw,ph]=value.split('x').map(Number);const scale=Math.min(1,8192/Math.max(pw,ph),Math.sqrt(24000000/(pw*ph)));assert.equal(elements.canvas.width,Math.round(pw*scale));assert.equal(elements.canvas.height,Math.round(ph*scale));assert(Math.abs(evaluate('width/height')-pw/ph)<1e-10);assert(Math.abs(evaluate('Math.min(width,height)')-720)<1e-8);context.formatProject=evaluate('projectData()');assert.equal(evaluate('validateProject(formatProject).resolution'),value);}
 ...
 console.log('PASS: all 19 format dimensions, aspect ratios, consistent scene scale, settings round-trip, portrait frame export.');
